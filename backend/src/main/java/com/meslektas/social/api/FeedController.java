...
public class FeedController {
    ...
    @Operation(
        ...
        description = "Returns paginated personalized feed based on following, profession, and engagement. " +
                      "Feed is cached for 5 minutes per user.",
    ...
    public ResponseEntity<ApiResponse<List<FeedPostResponse>>> getFeed(
            @Parameter(description = "Filter by profession ID (optional)")
            @RequestParam(required = false) Long professionFilter,
            
            @Parameter(description = "Max results (max 50)", example = "20")
        ...
        log.info("GET /api/feed - userId: {}, limit: {}, professionFilter: {}", 
            userId, limit, professionFilter);
    ...
    @Operation(
        ...
        description = "Returns trending posts from last 7 days ranked by engagement score. " +
