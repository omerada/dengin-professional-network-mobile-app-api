...
public class UserSanctionService {
     ...
     * Note: This implementation assumes we would add appeal functionality to UserSanction.
    ...
    public SanctionResponse submitAppeal(AppealRequest request, Long userId) {
        ...
        if (!sanction.getUserId().equals(userId)) {
            throw new BusinessException("ACCESS_DENIED",
                    "Bu yaptırım için itiraz yapma yetkiniz yok");
        }

        // Check if sanction is still active
        if (!sanction.isInEffect()) {
            throw new BusinessException("SANCTION_INACTIVE",
                    "Aktif olmayan yaptırıma itiraz edilemez");
        }

        // Note: In a full implementation, we would track appeal status in the UserSanction entity
    ...
    public SanctionResponse getSanction(UUID sanctionId, Long userId) {
        ...
        if (!sanction.getUserId().equals(userId)) {
            throw new BusinessException("ACCESS_DENIED",
                    "Bu yaptırımı görüntüleme yetkiniz yok");
        }
    ...
    public void validateUserCanAct(Long userId) {
        if (isBanned(userId)) {
            throw new BusinessException("USER_BANNED",
